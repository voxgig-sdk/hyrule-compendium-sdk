...
type Args struct {
	Entity string         `json:"entity" jsonschema:"category | compendium_entry | master_mode | region"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.HyruleCompendiumSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "hyrule-compendium_list",
		Description: "List records from HyruleCompendium. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "hyrule-compendium_load",
		Description: "Load a single record from HyruleCompendium. " +
...
func runOp(client *sdk.HyruleCompendiumSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.HyruleCompendiumSDK, name string) (sdk.HyruleCompendiumEntity, error) {
	switch strings.ToLower(name) {
	case "category":
		return client.Category(nil), nil
	case "compendium_entry":
		return client.CompendiumEntry(nil), nil
	case "master_mode":
		return client.MasterMode(nil), nil
	case "region":
