...
[MCP](https://modelcontextprotocol.io) server exposing the HyruleCompendium SDK as
two agent tools — `hyrule-compendium_list` and `hyrule-compendium_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hyrule-compendium-mcp)
make build

# 2. Provide credentials via the environment
export HYRULE_COMPENDIUM_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user hyrule-compendium \
  -- /absolute/path/to/hyrule-compendium-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./hyrule-compendium-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// hyrule-compendium_list: first page of records
{ "entity": "region" }
{ "entity": "region", "query": { } }

// hyrule-compendium_load: one record by id
{ "entity": "category", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hyrule-compendium-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user hyrule-compendium \
     -- "$PWD"/dist/*/hyrule-compendium-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `hyrule-compendium_list` and `hyrule-compendium_load` tools now appear
   in new sessions. Ask the agent to *"list region using hyrule-compendium"*
   and it calls `hyrule-compendium_list` with `{"entity":"region"}`.
...
```sh
export HYRULE_COMPENDIUM_APIKEY=sk_live_xxx            # API key
export HYRULE_COMPENDIUM_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `hyrule-compendium_list` tool
...
### Call the `hyrule-compendium_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `hyrule-compendium_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `hyrule-compendium_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HYRULE_COMPENDIUM_APIKEY` | API key sent with every request. |
| `HYRULE_COMPENDIUM_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

category | compendium_entry | master_mode | region

### Smoke test via HTTP (raw JSON-RPC)

```sh
./hyrule-compendium-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"hyrule-compendium_load","arguments":{"entity":"category","query":{"id":1}}}}'
```
