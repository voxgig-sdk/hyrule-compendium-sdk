...
(0, node_test_1.describe)('RegionDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-region', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-region', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.id = "hyrule_field";
        }
        else {
            params.id = 'direct01';
        }
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['HYRULE_COMPENDIUM_TEST_REGION_ENTID'];
