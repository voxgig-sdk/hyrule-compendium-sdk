...
(0, node_test_1.describe)('CategoryDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-category', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-category', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['HYRULE_COMPENDIUM_TEST_CATEGORY_ENTID'];
