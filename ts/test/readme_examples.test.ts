...
//     (\`new HyruleCompendiumSDK(...)\` / \`HyruleCompendiumSDK.test(...)\` become
//     \`HyruleCompendiumSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'HyruleCompendiumSDK'
...
const TEST_SEED = {"entity":{"category":{"test01":{"id":"test01"}},"compendium_entry":{"test01":{"id":"test01"}},"master_mode":{"test01":{"id":"test01"}},"region":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new HyruleCompendiumSDK(...)\` and any
// \`HyruleCompendiumSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+HyruleCompendiumSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/HyruleCompendiumSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
