...
describe('CategoryDirect', async () => {
  ...
  test('direct-load-category', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-category', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HYRULE_COMPENDIUM_TEST_CATEGORY_ENTID']
