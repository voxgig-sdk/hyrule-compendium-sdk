...
describe('CompendiumEntryDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new HyruleCompendiumSDK({
  ...
  test('direct-load-compendium_entry', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-compendium_entry', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HYRULE_COMPENDIUM_TEST_COMPENDIUM_ENTRY_ENTID']
