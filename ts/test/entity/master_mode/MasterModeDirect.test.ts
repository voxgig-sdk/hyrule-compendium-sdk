...
describe('MasterModeDirect', async () => {
  ...
  test('direct-load-master_mode', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-master_mode', setup.live)) return
    if (skipIfMissingIds(t, setup, ["entry01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HYRULE_COMPENDIUM_TEST_MASTER_MODE_ENTID']
