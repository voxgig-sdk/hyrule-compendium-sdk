...
module HyruleCompendiumUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = HyruleCompendiumResponse.new({})
    ...
    if fetch_err
      ...
      response = HyruleCompendiumResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = HyruleCompendiumResponse.new(fetched)
    else
