...
// HyruleCompendium SDK retry feature
...
class HyruleCompendiumRetryFeature extends HyruleCompendiumBaseFeature
{
    ...
    public function init(HyruleCompendiumContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HyruleCompendiumContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HyruleCompendiumContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HyruleCompendiumContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
