...
// HyruleCompendium SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class HyruleCompendiumTestFeature extends HyruleCompendiumBaseFeature
{
    ...
    public function init(HyruleCompendiumContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (HyruleCompendiumContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = HyruleCompendiumParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (HyruleCompendiumContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(HyruleCompendiumContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = HyruleCompendiumParam::call($ctx, $k);
