...
local CompendiumEntryEntity = {}
CompendiumEntryEntity.__index = CompendiumEntryEntity


function CompendiumEntryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CompendiumEntryEntity)
  self._name = "compendium_entry"
  self._client = client
...
function CompendiumEntryEntity:get_name()
  return self._name
end


function CompendiumEntryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CompendiumEntryEntity.new(self._client, opts)
end
...
function CompendiumEntryEntity:mark_deleted()
  self._deleted = true
end


function CompendiumEntryEntity:deleted()
  return true == self._deleted
end


function CompendiumEntryEntity:data_set(args)
  if args ~= nil then
...
function CompendiumEntryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CompendiumEntryEntity:match_set(args)
  if args ~= nil then
...
function CompendiumEntryEntity:match_get()
...
function CompendiumEntryEntity:stream(action, args, callopts)
...
function CompendiumEntryEntity:_run_op(ctx, post_done)
  local utility = self._utility
