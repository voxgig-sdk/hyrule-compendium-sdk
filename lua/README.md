...
The Lua SDK for the HyruleCompendium API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Category(nil):load({ id = "example_id" }, nil)
...
local result, err = client:HyruleCompendium(nil):load(
  { id = "test01" }, nil
)
