...
describe("CategoryEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set HYRULE_COMPENDIUM_TEST_CATEGORY_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    assert.is_not_nil(category_ref01_data_dt0_loaded)
...
function category_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("HYRULE_COMPENDIUM_TEST_CATEGORY_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["HYRULE_COMPENDIUM_TEST_CATEGORY_ENTID"])
