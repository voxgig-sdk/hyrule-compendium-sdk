...
local function make_error_util(ctx, err)
  ...
  local msg = "HyruleCompendiumSDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = HyruleCompendiumError.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == HyruleCompendiumError then
