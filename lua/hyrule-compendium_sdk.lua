...
require("hyrule-compendium_types")
...
local HyruleCompendiumSDK = {}
HyruleCompendiumSDK.__index = HyruleCompendiumSDK
...
HyruleCompendiumSDK._make_feature = _make_feature


function HyruleCompendiumSDK.new(options)
  local self = setmetatable({}, HyruleCompendiumSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function HyruleCompendiumSDK:options_map()
...
function HyruleCompendiumSDK:get_utility()
  return Utility.copy(self._utility)
end


function HyruleCompendiumSDK:get_root_ctx()
  return self._rootctx
end


function HyruleCompendiumSDK:prepare(fetchargs)
...
function HyruleCompendiumSDK:direct(fetchargs)
...
function HyruleCompendiumSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function HyruleCompendiumSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "HyruleCompendiumSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function HyruleCompendiumSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function HyruleCompendiumSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "HyruleCompendiumSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function HyruleCompendiumSDK:CompendiumEntry(data)
  ...
  if data == nil then
    if self._compendium_entry == nil then
      self._compendium_entry = EntityMod.new(self, nil)
    end
    return self._compendium_entry
  end
  return EntityMod.new(self, data)
end
...
function HyruleCompendiumSDK:MasterMode(data)
  ...
  if data == nil then
    if self._master_mode == nil then
      self._master_mode = EntityMod.new(self, nil)
    end
...
function HyruleCompendiumSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = HyruleCompendiumSDK.new(sdkopts)
  sdk.mode = "test"
