// Command HyruleCompendium-cli is a CLI + REPL for the HyruleCompendium SDK,
...
const prompt = "hyrule-compendium"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "category compendium_entry master_mode region"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HYRULE_COMPENDIUM_APIKEY (api key), HYRULE_COMPENDIUM_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HYRULE_COMPENDIUM_APIKEY carries the API key and
	// HYRULE_COMPENDIUM_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HYRULE_COMPENDIUM_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HYRULE_COMPENDIUM_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHyruleCompendiumSDK(opts)
