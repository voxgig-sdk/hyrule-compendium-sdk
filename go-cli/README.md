...
boru-driven command-line client **and** interactive REPL for the HyruleCompendium
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hyrule-compendium-cli)
make build

# 2. See usage (words, entities, env vars)
./hyrule-compendium-cli --help

# 3. Provide credentials once, via the environment
export HYRULE_COMPENDIUM_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./hyrule-compendium-cli load 1 category            # {id:1} shorthand
./hyrule-compendium-cli load '{id:1}' category       # explicit match map

# 5. Override the API base URL for a single call
HYRULE_COMPENDIUM_BASE=https://api.example.com ./hyrule-compendium-cli load 1 category

# 6. No arguments -> interactive REPL
./hyrule-compendium-cli
hyrule-compendium> load 1 category
hyrule-compendium> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hyrule-compendium-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/hyrule-compendium-cli load 1 category
   ```
...
```sh
./hyrule-compendium-cli load 1 category          # scalar shorthand for {id:1}
./hyrule-compendium-cli load '{id:1}' category     # explicit match map
```
...
./hyrule-compendium-cli load 1 category
```
...
Run with no arguments to open a REPL (prompt `hyrule-compendium>`). Each line is
evaluated as its own boru expression:

```text
$ ./hyrule-compendium-cli
hyrule-compendium> load 1 category
hyrule-compendium> /help
hyrule-compendium> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/hyrule-compendium-cli`. |
...
(`./hyrule-compendium-cli <expr>`) and interactively (the REPL), and expressions compose the
