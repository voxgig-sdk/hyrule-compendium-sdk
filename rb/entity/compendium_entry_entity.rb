...
class CompendiumEntryEntity
  ...
  def make
    opts = @_entopts.dup
    CompendiumEntryEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = HyruleCompendiumHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [CompendiumEntry, Hash] the current CompendiumEntry data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = HyruleCompendiumHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of CompendiumEntry fields)
  def match_get
  ...
  #
  # @param reqmatch [CompendiumEntryLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.CompendiumEntry.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [CompendiumEntry, Hash] the loaded CompendiumEntry; raises HyruleCompendiumError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = HyruleCompendiumHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
