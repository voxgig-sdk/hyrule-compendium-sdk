...
The Ruby SDK for the HyruleCompendium API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Category(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.HyruleCompendium(nil).load(
  { "id" => "test01" }, nil
)
