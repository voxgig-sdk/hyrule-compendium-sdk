...
class MasterModeEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set HYRULE_COMPENDIUM_TEST_MASTER_MODE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    master_mode_ref01_data_dt0_load_result = Helpers.to_map(master_mode_ref01_data_dt0_loaded.respond_to?(:data_get) ? master_mode_ref01_data_dt0_loaded.data_get : master_mode_ref01_data_dt0_loaded)
    assert !master_mode_ref01_data_dt0_load_result.nil?
    assert_equal master_mode_ref01_data_dt0_load_result["id"], master_mode_ref01_data["id"]
...
def master_mode_basic_setup(extra)
  ...
  entid_env_raw = ENV["HYRULE_COMPENDIUM_TEST_MASTER_MODE_ENTID"]
