...
class CompendiumEntryEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set HYRULECOMPENDIUM_TEST_COMPENDIUM_ENTRY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def compendium_entry_basic_setup(extra)
  ...
  entid_env_raw = ENV["HYRULECOMPENDIUM_TEST_COMPENDIUM_ENTRY_ENTID"]
