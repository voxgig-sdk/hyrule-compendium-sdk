...
class HyruleCompendiumSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = HyruleCompendiumHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = HyruleCompendiumHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, HyruleCompendiumFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    HyruleCompendiumUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = HyruleCompendiumHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = HyruleCompendiumHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => HyruleCompendiumError.new(
        "#{op}_allow",
        "HyruleCompendiumSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = HyruleCompendiumHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = HyruleCompendiumError.new(
        "graphql_error", "HyruleCompendiumSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = HyruleCompendiumSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
