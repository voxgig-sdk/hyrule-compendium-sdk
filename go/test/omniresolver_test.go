...
import (
	...
	omni "github.com/voxgig-sdk/hyrule-compendium-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.HyruleCompendiumError); is {
				out := map[string]any{
					"name":    "HyruleCompendiumError",
...
func omniCtx(arg any, client *sdk.HyruleCompendiumSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.HyruleCompendiumSDK); is {
			client = live
