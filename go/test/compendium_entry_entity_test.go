...
func TestCompendiumEntryEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HYRULECOMPENDIUM_TEST_COMPENDIUM_ENTRY_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		if compendiumEntryRef01DataDt0Loaded == nil {
...
func compendium_entryBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HYRULECOMPENDIUM_TEST_COMPENDIUM_ENTRY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HYRULECOMPENDIUM_TEST_COMPENDIUM_ENTRY_ENTID"])
