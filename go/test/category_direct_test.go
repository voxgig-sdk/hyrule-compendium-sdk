...
func TestCategoryDirect(t *testing.T) {
	t.Run("direct-load-category", func(t *testing.T) {
		setup := categoryDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-category", _mode); _shouldSkip {
			if _reason == "" {
...
func categoryDirectSetup(mockres any) *categoryDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"HYRULE_COMPENDIUM_TEST_CATEGORY_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["HYRULE_COMPENDIUM_TEST_CATEGORY_ENTID"]; ok {
