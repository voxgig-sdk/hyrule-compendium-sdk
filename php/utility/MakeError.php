...
class HyruleCompendiumMakeError
{
    public static function call(?HyruleCompendiumContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new HyruleCompendiumContext([], null);
        }
        $op = $ctx->op ?? new HyruleCompendiumOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new HyruleCompendiumResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof HyruleCompendiumError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new HyruleCompendiumError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof HyruleCompendiumError) {
