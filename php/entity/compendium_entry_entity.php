...
// HyruleCompendium SDK CompendiumEntry entity
...
class CompendiumEntryEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CompendiumEntryEntity($this->_client, $opts);
    }

    /**
     * @param CompendiumEntry|array $args CompendiumEntry data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = HyruleCompendiumHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return CompendiumEntry|array The current CompendiumEntry data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of CompendiumEntry fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = HyruleCompendiumHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of CompendiumEntry fields).
     */
    ...
    /**
     * Load a single CompendiumEntry.
     *
     * @param CompendiumEntryLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CompendiumEntryLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CompendiumEntry|array The loaded CompendiumEntry as an assoc-array at the
     *   SDK boundary; throws HyruleCompendiumError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = HyruleCompendiumHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
