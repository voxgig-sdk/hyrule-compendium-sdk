...
// HyruleCompendium SDK feature test
//
...
class FtCtrl extends HyruleCompendiumControl
{
...
class FtClient
{
    ...
    private HyruleCompendiumUtility $utility;

    public function __construct(HyruleCompendiumUtility $utility, array $options)
    {
    ...
    public function get_utility(): HyruleCompendiumUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HyruleCompendiumContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HyruleCompendiumContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HyruleCompendiumContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HyruleCompendiumConfig::make_feature($name);
    ...
    private static function buildUrl(HyruleCompendiumSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HyruleCompendiumError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HyruleCompendiumResponse($fetched);
        }
    ...
    private function fail(HyruleCompendiumContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HyruleCompendiumContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
