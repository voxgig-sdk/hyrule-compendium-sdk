...
//    only - a typed HyruleCompendiumContext there would make every ctx
...
final class HyruleCompendiumOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new HyruleCompendiumContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new HyruleCompendiumSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new HyruleCompendiumResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new HyruleCompendiumError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new HyruleCompendiumResponse($clean['response']);
    ...
    public static function err_from_map($m): ?HyruleCompendiumError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new HyruleCompendiumError(is_string($code) ? $code : '', $msg);
    }
