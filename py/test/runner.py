# HyruleCompendium SDK test runner
...
class HyruleCompendiumTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HyruleCompendiumTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HyruleCompendiumTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HyruleCompendiumTestRunner.getenv("HYRULE_COMPENDIUM_TEST_LIVE")
        override = HyruleCompendiumTestRunner.getenv("HYRULE_COMPENDIUM_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HyruleCompendiumTestRunner.getenv(key)
        ...
        explain = HyruleCompendiumTestRunner.getenv("HYRULE_COMPENDIUM_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if HyruleCompendiumTestRunner._test_control is not None:
            return HyruleCompendiumTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HyruleCompendiumTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HyruleCompendiumTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HyruleCompendiumTestRunner.load_test_control()
...
def load_env_local():
    HyruleCompendiumTestRunner.load_env_local()


def env_override(m):
    return HyruleCompendiumTestRunner.env_override(m)


def entity_data(v):
    return HyruleCompendiumTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HyruleCompendiumTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HyruleCompendiumTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HyruleCompendiumTestRunner.load_test_control()


def live_client_options():
    return HyruleCompendiumTestRunner.live_client_options()


def live_delay_ms():
    return HyruleCompendiumTestRunner.live_delay_ms()
