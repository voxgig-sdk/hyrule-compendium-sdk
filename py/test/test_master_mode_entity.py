...
class TestMasterModeEntity:

    def test_should_create_instance(self):
        testsdk = HyruleCompendiumSDK.test(None, None)
        ent = testsdk.MasterMode(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _master_mode_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "master_mode." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HYRULE_COMPENDIUM_TEST_MASTER_MODE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        master_mode_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.master_mode")))
        master_mode_ref01_data = None
        if len(master_mode_ref01_data_raw) > 0:
        ...
        master_mode_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(master_mode_ref01_data_dt0_loaded))
        assert master_mode_ref01_data_dt0_load_result is not None
        assert master_mode_ref01_data_dt0_load_result["id"] == master_mode_ref01_data["id"]



def _master_mode_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/master_mode/MasterModeTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HYRULE_COMPENDIUM_TEST_MASTER_MODE_ENTID"))
