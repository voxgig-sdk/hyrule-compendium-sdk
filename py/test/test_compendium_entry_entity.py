...
class TestCompendiumEntryEntity:

    def test_should_create_instance(self):
        testsdk = HyruleCompendiumSDK.test(None, None)
        ent = testsdk.CompendiumEntry(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _compendium_entry_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "compendium_entry." + _op, "live" if _live else "unit")
            if _skip:
        ...
        compendium_entry_ref01_data = None
        if len(compendium_entry_ref01_data_raw) > 0:
        ...
        compendium_entry_ref01_data_dt0_loaded, err = compendium_entry_ref01_ent.load(compendium_entry_ref01_match_dt0, None)
        assert err is None
        assert compendium_entry_ref01_data_dt0_loaded is not None



def _compendium_entry_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/compendium_entry/CompendiumEntryTestData.json")
